...
def train():
    ...
    ###========================== DEFINE MODEL ============================###
    t_image = tf.placeholder('float32', [batch_size, patch_size, patch_size, config.model.input_channel], name='t_image_input')
    t_target_image = tf.placeholder('float32', [batch_size, patch_size*config.model.scale, patch_size*config.model.scale, config.model.output_channel], name='t_target_image')
    ...
    log_dir = "%s/%s_log"%(config.model.log_path,tl.global_flag['mode'])
    ...
    for epoch in range(n_epoch_init):
        ...
        for idx_file in range(len(idx_list)):
            ...
            # print("Epoch [%2d/%2d] folder:%4d time: %4.4fs, mse: %.8f " % (epoch, n_epoch_init, idx_folder, time.time() - step_time, errM))
            total_mse_loss += errM
            n_iter += 1
        ...
        # if config.train.log_write is True:
        ...
        ## quick evaluation on sample set
        if True:#(epoch != 0) and (epoch % 1 == 0):
            print("[*] save images")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--mode', choices=['train','evaluate','test'], default='train', help='select mode')
